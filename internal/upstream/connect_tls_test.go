...
// The TLS floor contract for the CONNECT dialer's two config builders
// (code scanning go/missing-ssl-minversion, issue #18): both hops state
...
func TestOriginHopTLSFloor(t *testing.T) {
	t.Run("literal states the floor", func(t *testing.T) {
		cfg := (&connectDialer{}).originTLSConfig("origin.example:443")
		if cfg.MinVersion != tls.VersionTLS12 {
		...
		if len(cfg.NextProtos) != 2 || cfg.NextProtos[0] != "h2" || cfg.NextProtos[1] != "http/1.1" {
			t.Fatalf("origin-hop NextProtos = %v, want [h2 http/1.1]", cfg.NextProtos)
		}
	})

	t.Run("injected config inherits the floor", func(t *testing.T) {
		d := &connectDialer{tlsConfig: func() *tls.Config { return &tls.Config{} }}
		cfg := d.originTLSConfig("origin.example:443")
		if cfg.MinVersion != tls.VersionTLS12 {
			t.Fatalf("floored MinVersion = %x, want TLS 1.2 (an unset floor must be raised)", cfg.MinVersion)
		}
		if cfg.ServerName != "origin.example" {
		...
		if len(cfg.NextProtos) != 2 {
			t.Fatalf("clone NextProtos = %v, want the h2 + http/1.1 fill", cfg.NextProtos)
		}
	})

	t.Run("higher floor is never lowered", func(t *testing.T) {
		d := &connectDialer{tlsConfig: func() *tls.Config { return &tls.Config{MinVersion: tls.VersionTLS13} }}
		if cfg := d.originTLSConfig("origin.example:443"); cfg.MinVersion != tls.VersionTLS13 {
			t.Fatalf("MinVersion = %x, want the injected TLS 1.3 preserved", cfg.MinVersion)
		}
	})
}
