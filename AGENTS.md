...
| `internal/config`    | every runtime constant + the `OCFP_`-prefixed bootstrap env vars; multi-egress YAML model (routes, egresses, `upstream.base`, `auth.keys`, `user_agent.sync_interval`), interpolation, redaction, hot-reload store |
...
| `OCFP_CONFIG` | _(empty = built-in)_ | Multi-egress routing config file (YAML); hot-reloaded, invalid keeps last good; also carries `upstream.base`, `auth.keys`, `user_agent.sync_interval` |
